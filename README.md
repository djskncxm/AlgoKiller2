...
# 4. 多文件 trace（code.log + rw.log + bl.log 分离格式）
python run_algokiller.py --trace-dir ./trace/ --mode ciphertext
```
...
恢复时会沿用快照里的 `--trace-file`/`--trace-dir` 和 `--mode`，不允许覆盖。加 `-i` 可恢复后进入 REPL 而非自动 `continue`。
...
| 工具 | 职责 |
|------|------|
| `trace_search` | 大小写不敏感的精确子串搜索。必须二选一 `from_line`（向后）/ `before_line`（向前，最近优先）；`limit ≤ 100`。多文件模式下可通过 `file` 参数指定搜索 `code`/`rw`/`bl`，默认 `code`。十六进制查询未命中时自动尝试 endian 反序与 leading-zero 修剪 |
| `trace_context` | 按文件行号展开上下文，必须显式指定 `before` 与 `after`，各 `≤ 100`。多文件模式下可通过 `file` 参数指定文件 |
| `trace_cross_ref` | （多文件模式）给定 hex 序列号，一次返回该指令在 code.log、rw.log、bl.log 中的所有关联记录。内部使用二分查找，响应时间 < 1ms |
...
`tools/search/ak_search`（C，~1100 行）是核心性能基石：

- mmap 整个 trace，单次构建行偏移索引；
- ASCII 大小写不敏感的 BMH 搜索，不材料化 lowercase 副本；
- **daemon 模式**：harness 启动时为每个 trace 文件拉起一个常驻 `ak_search daemon`，所有 `trace_search` / `trace_context` 调用复用同一份内存映射与索引，不重复 IO；
- **序列号索引**（`--indexed-prefix`）：对 rw.log / bl.log 等分离文件，daemon 启动时额外构建 seq_id → 行号的有序索引，`seq_lookup` 命令通过二分查找实现 O(log n) 跨文件关联，响应时间 < 1ms（对比 BMH 全文搜索的 ~340ms）。

GB 级 trace 上 trace_search 的延迟通常在百毫秒量级；跨文件关联（trace_cross_ref）在亚毫秒量级。
...
接受 [GumTrace](https://github.com/lidongyooo/GumTrace) 生成的 ARM64 执行 trace。支持两种输入形式：

### 单文件格式（`--trace-file`）

所有数据交织在一个文件中，harness 强依赖以下行格式：
...
trace 目录下包含分离的日志文件，通过 hex 序列号（`hex_seq`）跨文件关联：

| 文件 | 内容 | 行格式 |
|------|------|--------|
| `code.log` | ARM64 指令执行 | `<hex_seq>  : <abs_pc>  [<rel_offset>]  "<disasm>" <reg_annotations>` |
| `rw.log` | 内存读写 hexdump | header: `<hex_seq>: (r\|w)(0x<base>+0x<offset>)` + hexdump 行 + 空行 |
| `bl.log` | 外部函数调用 | header: `<hex_seq>: [0x<arg_ptr>][<arg_idx>]: <func_name>` + hexdump 行 + 空行 |
| `arg.log` | 静态请求参数 | 自动注入 system prompt，无需手动搜索 |

- `hex_seq` 是从 0 开始的十六进制执行序列号，所有文件共享同一编号空间
- 一条 code.log 指令可以有 0+ 条 rw.log 记录（内存操作）和 0+ 条 bl.log 记录（外部调用）
- rw.log / bl.log 中的序列号是有序的，harness 利用二分查找实现高效跨文件关联
...
tools/search/
├── search.c                      C 实现：mmap + 行索引 + BMH + seq 二分索引，支持 match/context/seq_lookup/daemon
├── Makefile
├── build.sh
