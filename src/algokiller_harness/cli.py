...
def _build_agent_from_config(config: HarnessConfig) -> TraceAgent:
    ...
    executor = RoutingToolExecutor(
        trace_executor=LocalTraceToolExecutor(
            config.artifacts_dir, config.trace_file, trace_files=config.trace_files
        ),
        artifact_executor=artifact_executor,
        user_executor=UserInteractionToolExecutor(),
    )

    multifile = config.trace_files is not None
    if multifile:
        tools = [
            TRACE_SEARCH_TOOL_MULTIFILE,
            TRACE_CONTEXT_TOOL_MULTIFILE,
    ...
    if multifile:
        files_desc = []
        if config.trace_files.code:
            files_desc.append("code (instruction execution)")
        if config.trace_files.rw:
            files_desc.append("rw (memory read/write)")
        if config.trace_files.bl:
            files_desc.append("bl (external function calls)")
        bound_context_message = {
            "role": "system",
            "content": (
                f"Session trace directory: {config.trace_dir}. "
                f"Available trace files: {', '.join(files_desc)}. "
                f"Analysis mode: {config.mode}. "
                "trace_search and trace_context default to 'code'. "
                "Pass file='rw' to search memory read/write logs, file='bl' for external call logs. "
                "Use trace_cross_ref with a hex sequence ID to correlate records across all files."
            ),
        }
    else:
        bound_context_message = {
    ...
    if multifile and config.trace_files.arg and config.trace_files.arg.is_file():
        arg_content = config.trace_files.arg.read_text(encoding="utf-8", errors="replace")
        arg_message = {
            "role": "system",
            "content": f"Static request parameters (arg.log):\n{arg_content.strip()}",
        }
        agent.messages.append(arg_message)
        agent.startup_context_messages.append(arg_message)

    return agent


def build_agent(trace_file: str = None, trace_dir: str = None, mode: str = "ciphertext") -> TraceAgent:
    return _build_agent_from_config(load_config(trace_file=trace_file, trace_dir=trace_dir, mode=mode))
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--trace-dir",
        help="Directory containing split trace files (code.log, rw.log, bl.log, arg.log).",
    )
    parser.add_argument(
        "--mode",
...
def _apply_resume_session_args(args: argparse.Namespace, session_data: dict[str, object]) -> None:
    ...
    if args.trace_file and trace_file and str(Path(args.trace_file).expanduser().resolve()) != str(
    ...
    args.trace_file = trace_file if not trace_dir else None
